...
class Workbook(object):
    ...
    def individual_batting(self):
        ...
        df['person.ref'] = (~df['nameLast'].isnull()).cumsum().apply(lambda x: 'B%04d%d' % (x, damm.encode("%04d" % x)))
...
if __name__ == '__main__':
    ...
    year = sys.argv[1]
    books = [ fn for fn in glob.glob("transcript/%s/*.xls" % year)
              if not "~" in fn ]
    print "Processing source %s" % year
    for book in books:
        print "  %s" % book

    try:
        os.makedirs("processed/%s" % year)
    except os.error:
        pass
